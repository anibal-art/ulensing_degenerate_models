...
## Final pre-production integration and timeout policy

A final integration test was run on 50 physical catalogue events using the same
FAST production path intended for the full catalogue.

The test generated 100 logical datasets (H0 and H1 for each physical event).
Eighteen physical events passed the noise-independent detectability gate and 32
were rejected before fitting. H0/H1 pairing was exact for both noise seeds and
detectability status (50/50 pairs), and no negative LRT statistics were found.

For the 36 fitted logical datasets, the total wall-time distribution was
approximately:

- median: 91.85 s
- 95th percentile: 153.75 s
- maximum: 248.03 s

The slowest logical tasks were not dominated by the final H0/H1 TRF optimizer;
most of their excess wall time occurred in the broader additional-fit workflow.

For the initial full-catalogue FAST pass, a post-detectability wall-time limit
of 600 s per logical dataset is therefore adopted. This is deliberately
conservative relative to the observed timing distribution (~3.9 x q95 and
~2.4 x the observed maximum).

The timeout starts only after an event passes the pre-fit detectability gate.
A timed-out realization is recorded as `fit_timeout` with `needs_refit = true`.
It is never classified as a non-detection. Reproducibility information
(catalogue row, truth case, realization id, simulation seed, noise seed,
configuration and timeout metadata) is retained and timed-out realizations are
collected in `laggards.parquet` for a later dedicated refit pass.

The preliminary H0 sample from this integration test contained only 18
realizations (median T=2.16, q95=7.69, max=8.13) and is used only as a sanity
check. It is explicitly not the final calibration of T_alpha. The full
detectable H0 population will be used for that calibration.

A detailed record of the pre-production validation is stored in
